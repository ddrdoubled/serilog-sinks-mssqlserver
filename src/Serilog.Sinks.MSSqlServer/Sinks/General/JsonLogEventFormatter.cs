...
namespace Serilog.Sinks.MSSqlServer.Sinks.General
{
    ...
    public class JsonLogEventFormatter : ITextFormatter
    {
        ...
        ICollection<StandardColumn> store;

        private Func<StandardColumn, LogEvent, KeyValuePair<string, object>> standardColumnAndNameFunction;

        private ILogEventColumnOptions logEventColumnOptions;
        ...
        public JsonLogEventFormatter(
            ICollection<StandardColumn> standardColumnStore,
            Func<StandardColumn, LogEvent, KeyValuePair<string, object>> standardColumnAndNames,
            ILogEventColumnOptions logEventOptions)
        {
            store = standardColumnStore;
            standardColumnAndNameFunction = standardColumnAndNames;
            logEventColumnOptions = logEventOptions;
        }
        ...
        public void Format(LogEvent logEvent, TextWriter output)
        {
            ...
            if (logEventColumnOptions.ExcludeStandardColumns == false)
            {
                ...
                {
                    if(store != null && store.Contains(col))
                    {
                        output.Write(precedingDelimiter);
                        precedingDelimiter = COMMA_DELIMITER;
                        var colData = standardColumnAndNameFunction(col, logEvent);
